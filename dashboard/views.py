...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        aggregates = loans.aggregate(
            ...
            total_remaining=Coalesce(Sum('remaining_balance'), 0, output_field=DecimalField()),
            total_interest=Coalesce(Sum('total_interest_paid'), 0, output_field=DecimalField()),
        )
        ...
        prepay_stats = Prepayment.objects.filter(loan__user=user).aggregate(
            ...
            total_months_saved=Coalesce(Sum('months_reduced'), 0, output_field=DecimalField()),
        )
        ...
        if active_loan:
            ...
            labels = [f"M{r['month']}" for r in projected[:48]]
        ...
        ).select_related('loan').order_by('-payment_date')[:8]
